...
//
// Proposed lessons land as state='proposed'. Operator (Kernel)
// reviews evidence and either APPROVES (adds signal to next doctrine
...
export default function KernelReview() {
  ...
  const loadLessons = useCallback(async (state) => {
    ...
    try {
      const res = await api.get(
        `/admin/learning/lessons?state=${state}&limit=200`,
      );
  ...
  const loadAllCounts = useCallback(async () => {
    // One call per state — cheap, all keyed by state.
    const nextCounts = {};
    await Promise.all(
      STATES.map(async (s) => {
        try {
          const r = await api.get(
            `/admin/learning/lessons?state=${s.key}&limit=200`,
          );
  ...
  const sortedLessons = useMemo(() => {
    // Edge first (positive signals), then bleed; then by shrunk EV magnitude.

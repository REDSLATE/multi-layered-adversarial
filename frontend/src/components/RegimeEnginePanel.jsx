  ...
  if (lanes.length === 0) {
    return (
      <div className="text-xs text-zinc-500 mt-3" data-testid="regime-matrix-empty">
        ...
        Outcome Engine scores stamped intents.
      </div>
    );
  }
  return (
        ...
        return (
          <div key={lane}>
            <div className="text-xs uppercase tracking-wide text-zinc-500 mb-1">
              {lane} — brain × regime edge (prob-weighted return %)
            </div>
                ...
                <tbody>
                  {Object.entries(data.brains).map(([brain, row]) => (
                    <tr key={brain} className="border-t border-zinc-800"
                      ...
                      <td className="text-right py-1 pl-2 font-mono text-zinc-500">{row.n}</td>
                    </tr>
                  ))}
                </tbody>
              </table>

### 🛑 2026-08-05: EXIT-ONLY DIRECTIVE — "stop paying tuition while you diagnose why it loses"
- **OPERATOR DOCTRINE SHIFT**: live results negative (June −28.01, July −157.29, Aug −34.09 ≈ −219 broker-side). Directive: NO more threshold tuning to make it trade more; exit-only both lanes; closed-trade forensics not another rebuild; promotion gate before any automated entries return; canary-only road back.
- **Entry-mode governor** (`shared/execution_mode.py` + broker_router step 1c): modes live/exit_only/canary, **DEFAULT = exit_only, fail-closed** (missing/broken config can never re-enable entries — locked by test). Gate sits in `route_order` covering EVERY caller; scoped to BUY/SHORT only — SELLs/exits/manual/scanning untouched. Blocked fully-gated entries are recorded as **shadow fills** (`shadow_fills`, idempotent, hypo price = confirmation→mid→signal ladder) AND flow into the missed-entry ledger (scope extended to broker_reason exit_only_mode) → forward expectancy accrues automatically while flat. Canary = N trades/day cap (knob, default 3), count from executions.
- **Promotion gate** (`shared/forensics/promotion_gate.py`): forward-recorded shadow signals scored per lane on 5 criteria: n≥30, expectancy>0 after cost (0.30% knob), PF>1.1, maxDD≤10pct-pts, single-trade share≤0.5. POST /api/admin/entry-mode to live/canary **returns 409 while gate red** unless override=true (audited). GET /api/admin/entry-mode/promotion-gate.
- **Closed-trade forensics** (`shared/forensics/trade_forensics.py`, GET /api/admin/forensics/closed-trades): every outcome since 2026-06-01 classified into the operator's 4 loss mechanisms — bad_selection / late_entry / execution_cost / exit_policy (+winner/unknown), MFE/MAE from stored 5m else Kraken 1h/4h backfill (~120d reach), groupings by lane/brain/exit_reason/entry_source/month, dominant_loss_mechanism, monthly internal-vs-broker reconciliation (operator posts broker figures via POST /forensics/broker-actuals — June/July/Aug already stored). **Preview DB has NO live trades (prod DB holds them) — report shows 'run in PRODUCTION' note; real numbers appear after redeploy.**
- **Entry-latency report** (GET /api/admin/forensics/entry-latency?n=50) per operator hypothesis "it buys after the gain": per-trade signal→intent→submit→fill deltas, price@signal vs fill (chase%), high/low next 30min after fill, % bought within 1% of 30m top; cadence facts included: **MC Pulse 15s tick, momentum scanner 60s, universe refresh 15m** → polling was never the 15-30min lag; universe-membership discovery was (ignition watch 08-04 closes it); this report verifies on forward trades. Momentum scanner now stamps `signal_bar_ts`/`signal_price`/`signal_detected_at` on emitted intents so the full chain is measurable going forward.
- **UI**: `EntryModePanel` at top of Operator Control — red EXIT-ONLY banner, mode buttons (canary/live confirm + 409-driven override flow), shadow-fill count, per-lane GATE MET/NOT MET chips with all criteria (testids entry-mode-panel/-badge/-set-{mode}/-msg, promotion-gate-summary/-{lane}). `ForensicsPanel` below SellPointPanel — run report, buckets, dominant mechanism, monthly recon rows with Δ, broker-actuals input, entry-latency strip (forensics-panel/-run-btn/-summary/-buckets/-recon/-latency/-actuals-input/-actuals-save/-dominant/-note).
- Route table 525→531 (fixture regenerated). 10 new tests (`test_exit_only_forensics.py`) + 123 batch pass. Live-verified: default exit_only, 409 guard, override path, shadow fill written via real gate call (validation-exitonly-1), latency n=50 on old executions, UI screenshot. **Testing agent iteration_36: 15/15 pass, no action items.**
- **DEPLOY REQUIRED**: prod is still trading the old build. On redeploy prod boots exit_only automatically (code default) AND the forensic/latency reports run on prod's real June+ receipts.

### 📉 2026-08-05: SELL-POINT WATCHER (v3.5 plan item 5, operator approved)
- `shared/exits/pattern_watch.py`: fractal pivots (k=2) + ATR(14) on 5m bars for symbols with ACTIVE exit plans. Detectors: **double_top** (2 pivot highs within 0.5×ATR, close < valley neckline), **head_shoulders** (3 pivot highs, middle max, shoulders within 1×ATR, close < min retrace low), **rising_wedge** (rising converging pivot trendlines — lows steeper than highs — + volume fade <0.8 early-third + close breaks lower trendline). Recency: last peak within 15 bars.
- **OBSERVE-FIRST doctrine (locked by test)**: ships mode="observe" — detections write receipts to `sell_point_events` (permanent) only. mode="act" applies per-pattern action knob (off|tighten|exit, defaults tighten): **tighten** = stop → close − 0.5×ATR (knob stop_buffer_atr), NEVER lower than current stop, never at/above price, via plan_store.update with levels_source="sell_point_watch"; **exit** = exit monitor's close_now ladder. Dedupe: 60-min cooldown per (plan, pattern). Tape Quality Gate protects every evaluation. Worker every 120s (knob) in lifespan.
- Knobs `runtime_flags._id=sell_point_watch` {enabled, mode, interval_sec, cooldown_min, stop_buffer_atr, actions{...}}. Routes: GET/POST `/api/admin/sell-point` (Literal-validated), POST `/run-once`. Route table 522→525, fixture regenerated.
- UI `SellPointPanel.jsx` in Operator Control below ExitMonitorPanel: OBSERVE/ACT badge, "ARM ACT MODE" with window.confirm guard, per-pattern action selects (save on change), stop-buffer input, recent events feed / empty state (testids sell-point-panel/-mode-badge/-mode-toggle/-action-{pattern}/-buffer/-events/-event-{id}/-empty/-msg).
- 6 tests (`test_sell_point_watch.py`): pivots+ATR, double-top hit + flat-tape clean, H&S head-is-middle, wedge needs fade AND break (flat volume → None), tighten-never-lowers/never-at-price, wiring + observe-default lock. 53 batch pass. Live: GET/POST/run-once verified (0 active plans in preview → idles at plans:0), panel screenshot green.
- NOTE: candle-confirm upgrades (shooting_star at 2nd peak → tighten→exit etc, addendum item 2) NOT built — needs `shared/market_data/candles.py` which doesn't exist yet; propose as follow-up.

### 🎛️ 2026-08-04 (cont): Tape Knobs Panel + Alert History · ACCOUNT GUARDIAN CANCELLED
- **OPERATOR DECISION — Account Guardian is OFF the roadmap**: "We don't need this. May cause problems with the existing kill switch." Do NOT propose or build it again; the Forven plan section A is dead. (A2 Tape Quality Gate shipped; A3/B remain parked options only.)
- **Tape Knobs Panel**: `routes/tape_quality_admin.py` — GET/POST `/api/admin/tape-quality` (validated knobs: min_bars 3-60, min_completeness 0.5-1, max_gap_bars 1-120, max_staleness_tf_mult 1-60, enabled). UI `TapeQualityPanel.jsx` in Operator Control (below WatchlistPanel): ENFORCING/DISABLED badge, enable/disable toggle, 4 knob inputs + SAVE (testids tape-quality-panel/-status/-toggle/-save/-msg, tape-knob-*). Knobs read live by the gate (no restart).
- **Alert History**: MissedEntryTile gained a HISTORY toggle (missed-entry-history-toggle) → 30d browsable feed via existing `GET /api/admin/alerts?hours=720&include_acked=true` — acked/open chip, date, message, acked-by (missed-entry-history/-history-{id}/-history-empty). Ack actions refresh history live.
- Route table 520→522 (fixture regenerated). Verified: curl GET/POST tape knobs (persist + revert), history query returns the acked ETH alert, UI screenshots green (panel renders with defaults; history shows "ACKED · 2026-08-04 · ETH/USD would have hit +5% TP · by admin"). 14 tests in batch pass.

### 🚨 2026-08-04 (cont): MISS ALERTS (in-app, operator option A) + TAPE QUALITY GATE (Forven plan A2)
- **Miss Alerts**: missed-entry ledger now writes a `costly_miss` alert to NEW `operator_alerts` collection (permanent, idempotent `_id=alert-miss-{intent_id}`, `$setOnInsert`) whenever a blocked BUY's counterfactual = tp_hit. Routes `GET /api/admin/alerts?hours=&include_acked=` + `POST /api/admin/alerts/ack {alert_id|all}` (`routes/operator_alerts.py`, generic feed — future producers reuse it). UI: red pulsing "COSTLY MISS · n unacked" strip on MissedEntryTile with per-alert ACK + ACK ALL (testids missed-entry-alerts/-ack-{id}/-ack-all). Live-proven: seeded validation-missed-demo-2 (ETH blocked 6% below real path) → run-once → tp_hit → alert "ETH/USD would have hit +5% TP (peak +6.67%)" → UI strip rendered → ACK click cleared it.
- **Tape Quality Gate** (`shared/market_data/tape_quality.py`): `assess(bars)` infers tf from median bar spacing → verdicts thin_tape / stale_tape (age > 5×tf, knob) / gappy_tape (max interior gap > 12 bars) / incomplete_tape (completeness < 0.95) + fingerprint {n, span_min, last_ts}. Knobs `runtime_flags._id=tape_quality`. THREE call sites: (1) momentum scanner — granular reject pre-scoring (fail-open on gate errors); (2) `entry_timing.check_buy_entry` — bad tape → fail-CLOSED `BAD_TAPE_QUALITY` REJECT (receipt carries tape_quality; deliberately NOT in REARMABLE_REASONS — a data fault must never open a re-arm window); good verdicts get tape fingerprint stamped too; (3) `entry_rearm._tick` — skip-with-reason into `last_check` instead of advancing the state machine on bad bars.
- **Live behavior observed (preview, weekend tape)**: scanner blocks now read `gappy_tape:10 · stale_tape:7 · momentum_score_below_minimum:2` — thin weekend alt tapes correctly rejected pre-scoring (gap = no trades = no momentum anyway; an igniting symbol prints every minute). If prod over-blocks, tune `tape_quality` knobs (max_staleness_tf_mult / max_gap_bars / min_completeness) — no UI knob yet, POST runtime_flags or add panel later.
- Route table 518→520 (fixture regenerated). 11 new tests (`test_tape_quality_alerts.py`: all 4 failure modes + clean/5m-inference/knobs, entry-timing BAD_TAPE_QUALITY block, rearm-reason lock, alert idempotency + message, wiring). 38+66 regression pass. Self-tested end-to-end (curl chain + UI ack interaction + screenshot) — testing agent not used this round.

### 🔥 2026-08-04: P0 scanner crash fixed + IGNITION WATCH + MISSED-ENTRY LEDGER (operator: "still missing the entry point and completely missing the momentum read" → approved builds #1 & #2 from architecture review)
- **P0 ROOT CAUSE of "completely missing the momentum read": `CRYPTO_SCAN_CAP` was used in `_lane_symbols` but never defined** (left dangling by the 08-03 hybrid-eligibility session) — the crypto momentum lane crashed with NameError EVERY 60s cycle; backend logs showed thousands of loop errors. Fixed (`CRYPTO_SCAN_CAP = 30`); scanner immediately came back to life and EMITTED a real momentum intent (TAKE/USD) on the first clean cycle. Regression-locked in tests.
- **Ignition Watch (`momentum/ignition_watch.py`)** — closes the discovery gap (ICNT class): ONE bulk public Kraken Ticker call per scan cycle over ALL online USD pairs, ranked by per-minute 24h-dollar-volume DELTA between sweeps (needs 2 samples; first sweep = baseline). Positive volume-rate ≥ knob AND positive price move → top-N candidates fed into the crypto lane as `origin="ignition"` (auto-mapped via kraken_pair_sync + one-shot 1m bar backfill since they're outside the feeder universe). Eligibility rules + $5/trade cap still decide what trades. Knobs on `runtime_flags._id=momentum_scanner`: ignition_enabled(True)/ignition_top_n(5)/ignition_min_vol_usd_min($10k) — POSTable via /api/admin/momentum-scanner. Stables/fiat bases excluded. Fail-soft: sweep failure never stalls the universe scan. Live-verified: sweep logged `ETH/USD $78841/min +0.004%` on real tape.
- **Missed-Entry Ledger (`shared/risk_sizer/missed_entries.py` + `routes/missed_entries_admin.py`)** — the feedback loop now learns from trades that were BLOCKED: 4h (knob) after every blocked BUY (scope: `entry_timing:*` + opportunity-class `risk_sizer:*` — allowlist/volume/spread/no_quote/denylist/cooldown/cap-below-min; NEVER mechanical rejects like sized_to_zero), computes counterfactual peak/trough/end % + would-be tp_hit/sl_hit/expired vs the REAL exit levels (momentum-stack intents use momentum tp/sl; else lane exit policy). First-touch bar walk; TP+SL in same bar → sl_hit conservative + ambiguous flag. Crypto bars on-demand-backfilled from Kraken 5m (60h reach). Rows PERMANENT in `missed_entry_outcomes` (not in retention). Worker every 600s in lifespan. Routes: GET/POST `/api/admin/missed-entries`, POST `.../run-once` (idempotent). Observe-only — never gates.
- **UI**: MomentumScannerTile — amber IGN badge on ignition-origin candidates, ignition-sweep strip (ms-ignition-sweep), footer shows ignition ON/off. NEW MissedEntryTile on Overview (missed-entry-tile/-summary/-reasons/-recent/-outcome-{id}): blocked-BUY count, would-be TP rate, per-reason avg peak/end, recent misses with outcome chips.
- Route-table snapshot regenerated (→518). 12 new tests (`test_ignition_missed.py`: pure ranking/elapsed scaling/red-volume exclusion, scope filter, block-price ladder, tp/sl/ambiguous/expired outcome math, wiring locks); 36 in batch pass. Live proofs: seeded validation blocked-intent (validation-missed-demo-1, BTC @ 63982 5h ago) → run-once evaluated with 26 real bars → expired, peak +0.575%, end +0.494%. Testing agent iteration_35: 8/8 pass, 100%, no action items.
- **GOTCHA (4th occurrence of the parallel-edit failure class):** the DEFAULTS ignition-knob edit to momentum_scanner.py reported success but silently didn't persist — caught via live curl (config lacked keys), re-applied solo. Pattern holds: grep-verify parallel edits to momentum/exit files.
- **Needs deploy**: prod's scanner only reads the 9-symbol static allowlist, so it misses every off-list ignition. Next deploy carries: hybrid eligibility + $5 cap + scanner fix + ignition watch + missed-entry ledger + momentum equity lane.

### 💵 2026-08-03: $5/trade cap on EVERY crypto BUY (operator: "keep it low, like $5 per trade")
- Operator choices: cap applies to ALL crypto BUYs — pins included (not just rule-admitted coins); cap is an ADJUSTABLE knob, not hardcoded.
- `buy_eligibility.py`: knob renamed `max_notional_offlist_usd` → **`max_notional_usd`, default $5**. Every allowed verdict (static-allowed, operator_pin, rules_admitted) now carries `notional_cap_usd`; rules path still further clamps to ≤0.5% of 24h volume. Sizer clamps `final_notional` to the cap and stamps `eligibility_cap_usd` + `eligibility_cap_applied` on the receipt. Guard unchanged: cap < broker min ($5 crypto) → clean reject `eligibility_cap_below_broker_min` (so $5 cap == $5 min trades exactly at $5).
- UI: BuyAllowlistPanel (Kraken Universe page) gained a **Dynamic Eligibility** section — mode select, amber "MAX $X / TRADE — ALL BUYS" badge, editable knobs (max $/trade, min 24h $vol, max spread bps, % of 24h vol) + SAVE. testids: buy-eligibility-section/-mode-select/-mode-badge/-cap-badge/-max-notional/-min-dvol/-max-spread/-max-pct/-save-btn/-msg.
- **P0 caught en route: `routes/universe_admin.py` was missing `from pydantic import BaseModel, Field`** (previous session's EligibilityKnobs) — backend crashed on reload; would have bricked the next prod deploy at boot. Fixed.
- Stale fixtures healed: test_risk_sizer wired fixture now stubs eligibility + sell-cooldown (live preview SELLs were tripping post_sell_cooldown inside math tests); test_operator_fixes_0728 allowlist test pinned to static mode; not-bumped test updated to $5 doctrine. 172 sizer/allowlist/eligibility/risk/rearm/capital tests pass.
- Verified live: GET/POST /api/admin/universe/buy-eligibility (default 5.0, knob persisted with audit stamps), probe BTC/USD → operator_pin + cap 5.0, UI screenshot green.
- Needs deploy to reach prod (carries momentum equity lane + hybrid eligibility + this cap + the pydantic import fix).

### 📋 2026-08-02 (cont): v3.5 (JARVIS) + bearish-pattern sell-points PLANNED & PARKED
- Operator uploaded `risedual_v3_5_merged.py` (2118-line rewrite) + bearish pattern sketch (double top / H&S / rising wedge) asking "can this help?" — both planned, parked until after Monday per operator instruction. Full plan: **/app/memory/v35_pattern_adoption_plan.md**.
- Adopting (as drop-ins, NOT a rewrite): AdaptiveKernel per-stack weights from realized outcomes; OpportunityEngine as PARALLEL "opportunity" stack (top-N universe ranking, disagreement=multiplier never veto, no HOLD-by-default at signal layer); CapitalPressure (rotate capital from weakest position via exit monitor, max 3/day); economic-fingerprint dedup; Sell-Point Watcher (bearish patterns on HELD tickers → tighten stop/exit via monitor, observe-first).
- Best-judgment calls (operator skipped scope ask): plan-only now; parallel source before arbiter cutover; SAFETY gates (risk/allowlist/entry-timing/broker) untouched — "no confidence gates" applies to signal layer only.
- NOT porting: v3.5's parallel execution engine/broker router/portfolio state/toy models; kill-switch folds into planned Account Guardian.

### 📋 2026-08-02 (cont): Forven adoption PLANNED & PARKED (operator: "park until after tomorrow, plan it out, drop in later")
- Studied https://github.com/judder659/Forven (AGPL — reimplement concepts only, never copy code). Full drop-in plan written to **/app/memory/forven_adoption_plan.md**.
- Agreed build order AFTER Monday's live observation: **A. Account Guardian** (drawdown kill-switch vs HWM → auto broker-freeze + close-all + master OFF, manual re-arm only; daily loss limit; per-source loss-streak cooldown) → **A2. Tape Quality Gate** (completeness/gap/staleness checks for scanner + entry timing + re-arm; fixes today's no_tape/NO_TIMING_DATA class) → **A3. Failed-Open Latch** (block symbol BUYs until ambiguous broker orders reconcile) → **B. Replay Gauntlet** (knob-jitter + cost-stress replay of entry timing + momentum policies over stored bars; DSR later).
- NOT adopting: their agent team/strategy generation, exchange layer, paper sim, dashboard, storage, MCP/Discord/scheduler/data-lake, OKX liquidation feed (parked), builtin strategies.

### 🚀 2026-08-02: MOMENTUM POSITION CONTROLLER integrated (operator zip) — 5th signal source, live entries + fixed exits, crypto NOW / equity ready for next deploy
**Operator uploaded `risedual_momentum_controller.zip` ("Try this"): momentum entry source emitting BUY intents into the existing pipeline + fixed exits (+5%/−3% from broker average fill, reduce-only, idempotent). Integrated per the package's OWN rules: entries ride the normal gate chain, NO second broker path — exits execute through the existing exit monitor.**
- **`/app/backend/momentum/` (NB: named `momentum`, NOT `trader` — `/app/trader/` is the standalone trader service and a `backend/trader/` shadow package broke its imports; renamed same-session):** `momentum_position_controller.py` (operator's controller verbatim: EntryPolicy score≥0.60/Δ≥0.08/VWAP+EMA9/accel/spread 75bps eq·250bps cr/quote≤2.5s/rvol≥1.2/chase≤0.75%; evaluate_fixed_exit ±5/3 from avg fill) + `momentum_scanner.py` (the live wiring).
- **Scanner loop** (lifespan task, 60s, ships DISARMED): lanes crypto+equity. Crypto = BUY-allowlist symbols (no churn on unroutable pairs); equity = live_universe movers cap 30, RTH-gated via `is_equity_rth`, fails closed on missing quotes (`no_quote`). Tape math: `momentum_score` = tanh-compressed (drift·6 + thrust·15 + rvol kicker) — tanh because hard clamp saturated and killed the DELTA (transition) signal; `confirmation_price` = close of the bar where score last crossed UP through min_score, so stale mid-trend momentum rejects as `maximum_chase_exceeded`. Dedupe: skip if position held (active exit plan) or momentum intent within cooldown 30m; max 2 emits/cycle.
- **Emission**: `stack="momentum"` added to IntentIn Literal (5th source; `canonicalize_stack` passes it; Seat fires it at 50% governor multiplier — council-participant path). Emits via `submit_intent_in_process` with `doctrine_snapshot={bid,ask}` so entry timing gets a frozen confirmation. ALL gates re-run (seat/risk/allowlist/entry-timing/broker).
- **Exits**: existing `_submit_exit` machinery is the broker mapping (Webull `submit_close_market`, Kraken market/marketable-limit SELL w/ MC receipt, escalation, reconcile; qty ceiling = broker-held = functional reduce-only; `exit-{plan}-{attempt}` idempotent). NEW: `_adopt` momentum-origin override → TP basis×1.05 / SL basis×0.97, `levels_source="momentum_policy"` (knob-able tp_pct/sl_pct). Operator must keep Exit Monitor lane policies ENABLED.
- **Admin**: `GET/POST /api/admin/momentum-scanner` (arm/disarm, interval, cooldown, emit cap, lanes, tp/sl, score knobs; knob doc `runtime_flags._id=momentum_scanner`, state doc `momentum_scanner_state`). Route in registry manifest; route-table snapshot regenerated 508→512 (also includes the 2 entry-timing prod-watch routes).
- **UI**: `MomentumScannerTile.jsx` on Overview (testids momentum-scanner-tile/-toggle, ms-evaluated/-emitted/-total/-candidates/-rejections/-footer): ARMED toggle, per-symbol lane+reason+prev→cur score, rejection histogram, lanes+exit policy footer.
- **LIVE-verified in preview (armed)**: crypto cycle evaluates all 9 allowlist symbols — flat weekend tape correctly rejected `momentum_score_below_minimum` (scores .47–.51 vs .60); equity lane correctly `equity_market_closed` (Saturday). 20 momentum tests + 78 in wider batch pass. Preview left ARMED (harmless: master switch OFF).
- **Deploy state**: operator ALREADY deployed the crypto-era build to prod and armed it; the EQUITY lane update rides the NEXT deploy ("once I see it working with crypto"). Equity live behavior observable Monday RTH.
- Fixture healed en route: `test_universe_quality_knobs` (pre-07-28 $1 floor, pre-dvol volumes).

### 📡 2026-08-01 (cont): PROD DEPLOY WATCH shipped — first-organic-re-arm timeline, child outcome buckets, P0 deployment guards
**Operator sequencing locked: 1) prod re-arm proof → 2) emit-side damper → 3) sessions of timing outcomes → 4) evidence-based cap tuning → 5) re-arm alerting. Final proof = organic REARMED → ROUTED → BROKER_ACKNOWLEDGED (or clean BROKER_REJECTED). No cap tuning, no damper until then.**
- **`shared/risk_sizer/rearm_report.py` (new):** `classify_child_outcome` → waiting | blocked_again | submitted | filled | broker_rejected | reconciliation_required (submitted >5min unfilled or expired_unrouted → reconcile); `first_organic_rearm` (validation artifacts filtered by intent-id prefix/stack/rationale markers); `build_rearm_timeline` — the full linked chain: original intent (confirmation price+source, extension vs class cap) → trigger states → pullback receipt (depth/vol-contraction/support/reaccel) → child (lineage ids, new confirmation/invalidation/stop, seat tier, risk_sizing, allowlist verdict, second entry-timing verdict) → local-queue proof → executions → fills → improvement%; `health_checks` — the 3 P0 guards.
- **Routes (universe_admin, under /api/admin/universe/):** `GET /entry-timing/rearm-timeline?trigger_id=`, `GET /entry-timing/health`; `/entry-timing/stats` extended per window: organic_buy_intents, confirmation_sources{snapshot_price|snapshot_bid_ask_mid|bar_at_confirmation|missing}, blocks_by_class, rearm_children{created,in_local_queue,routed,outcomes}, duplicate_suppressions; top-level `first_prod_rearm`.
- **P0 guards:** (1) organic BUY hit NO_TIMING_DATA in 24h → derivation failing; (2) REARMED child (23h horizon = queue prune window) absent from local durable queue via new `intent_queue.has()` → will never route; (3) freeze doc frozen=false w/ lingering reason OR roadguard_hard_block while unfrozen. Guards LIVE-verified: they correctly flagged today's PRE-fix organic failures (2 AAPL NO_TIMING_DATA + roadguard blocks) — self-clear as the 24h window rolls — and caught a Mongo-only demo child as "missing from queue".
- **`entry_rearm.create_trigger`:** repeated block on a WATCHING symbol now `$inc duplicate_blocks_prevented` + `$max peak_price` (was silent return) — feeds the duplicate_suppressions stat.
- **UI `EntryTimingTile.jsx` rebuilt:** green pulsing **FIRST PROD RE-ARM · {symbol}** banner (testid et-first-rearm-banner) → click expands the 5-section timeline (et-rearm-timeline); outcome chip strip (et-rearm-{outcome}, queued x/y · routed z); red P0 GUARD alert rows (et-health-alerts, et-health-{id}); footer: organic BUYs · conf-src distribution · dup suppressed · "P0 guards OK" when green.
- **Testing:** 12 new tests (`test_rearm_prod_watch.py`: outcome buckets incl. reconciliation grace, vocabulary lock, validation filter, queue.has, duplicate-block recording, route registration); stale fixtures healed (`test_universe_quality_knobs.py` — pinned pre-07-28 $1 floor + pre-dvol volumes); 144 pass in the rearm/router/ledger/universe/freeze batch. Curl-verified all 3 endpoints; 2 UI screenshots (guards red on pre-fix history; banner+timeline verified via temp demo docs, cleaned up).
- **Preview posture:** master switch OFF, watcher alive, validation triggers left on tile (TTL 7d).
- **DEPLOY runbook:** deploy → tile shows P0 GUARDS OK (or names the failing fix) → wait for first organic chase-block → WATCHING appears → on re-arm the green banner + timeline self-report. Prod proof threshold: outcome reaches submitted/filled or broker_rejected.

### ✅ 2026-08-01: Trigger Watcher LIVE VALIDATION — 3 P0s caught & fixed, full state machine proven on live data
**Operator directive: "validate the watcher before anything else — it determines whether the momentum-entry fix recovers opportunities instead of merely blocking them."**
**Validation verdict: state machine + receipts match doctrine exactly, but 3 bugs would have broken it in prod — all found by driving REAL intents through the REAL router against LIVE Kraken bars, all fixed + regression-tested (106 pass).**
- **P0 #1 — confirmation price never existed on organic intents:** arbiter `_emit_intent` sends no snapshot; ingest enrichment freezes bid/ask but never a `price` key → EVERY organic BUY would hit `NO_TIMING_DATA` fail-closed REJECT (and NO_TIMING_DATA never re-arms) — total BUY blackout on deploy. FIX (`entry_timing.py::evaluate`): derive frozen confirmation = snapshot.price → bid/ask mid → close of bar at/before ingest_ts; receipt stamps `confirmation_source`; still fail-closed when truly no data.
- **P0 #2 — re-armed child intents could never route:** `_emit_child_intent` inserted into Mongo only, but the router picks from the LOCAL durable queue (Atlas is error-fallback only, 2026-07-24 hot-path) → children silently `expired_unrouted`. FIX: `intent_queue.enqueue_safe(child)` after insert; child also strips stale routing fields (route_timeouts, execution stamps).
- **P0 #3 — freeze→thaw permanently bricked BUYs:** `thaw()` never cleared `reason` in `broker_freeze_state`; 20s policy-snapshot refresh re-read it; sizer roadguard blocked on reason-TRUTHINESS not `broker_frozen` → after ANY freeze/thaw cycle every plan died `roadguard_hard_block` (live in preview via pytest residue; prod landmine). FIX both sides: thaw `$set reason:None`; sizer checks `broker_frozen`.
- **Live proofs (preview, real `_route_one`, live bars):** PASS path (fresh mid → `ENTRY OK +0.0%` → died only at missing preview Kraken keys); BLOCK path (stale quotes +5.5% > crypto 4% cap → `MISSED_ENTRY_CHASE_RISK` + WATCHING trigger w/ lineage + 4h expiry); WATCH polling on 6 live symbols every 30s (receipts show depth/vol-contraction/ema9/vwap/wait-reason — full doctrine vocabulary observed on 60 live symbols); REARM (controlled tape: 6.19% pullback, 0.47 vol contraction, support held, reaccel bar → REARMED, child @ 1.262 vs 1.30 block = +2.9% better entry, fresh invalidation=stop, router re-picked child, seat+risk re-ran, **allowlist correctly rejected the fake symbol** — protections re-run confirmed); EXPIRED (`window_closed_no_pullback`); duplicate triggers prevented (1→1, max_attempts=1).
- **Stats/UI verified:** `GET /api/admin/universe/entry-timing/stats` (NOTE: prefix is /admin/universe/, handoff had it wrong) — blocked-late 4, watching 6/rearmed 1/expired 1, re-entry improvement +2.923%; EntryTimingTile green on Overview; kill-map shows ENTRY_TIMING_REJECTED rows.
- **Stale fixtures updated:** `test_capital_ledger_wiring.py` (tier-doctrine confidence/ingest_ts + timing-gate stub), `test_risk_engine_lanes.py` roadguard test now pins frozen-flag semantics + thawed-reason-must-not-block.
- **Preview restored:** master switch OFF, synthetic bars removed. Validation triggers/intents left visible on tile (TTL 7d).
- **NOT verifiable in preview:** broker submits/fills on re-armed children (no Kraken/Webull keys). First prod watch: `entry_rearm_triggers` states + tile `RE-ARM FILLED` counter.
- **Prod runbook: DEPLOY REQUIRED — without fixes #1/#2 the gate would block every organic BUY and re-arms would never route.**

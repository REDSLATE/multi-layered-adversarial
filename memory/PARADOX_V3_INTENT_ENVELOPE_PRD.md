...
| 5.b | ✅ Shipped DORMANT | 2026-02-22 | Re-injection of fired plans into the unified pipeline behind `PARADOX_V3_TRIGGER_REFIRE` (default OFF). When ON: fired WAIT plan has `plan.intent` flipped WAIT→ENTER, `execution.action` synthesised from `plan.stance` per the locked table (BULLISH/LONG_BIAS → BUY both lanes; BEARISH/SHORT_BIAS → SHORT on equity, REFUSED on crypto since Kraken spot can't short; NEUTRAL → REFUSED). The mutated intent then runs through `run_unified_for_intent` so the seat re-evaluates conf_min + live consensus AT trigger-fire time. Status endpoint surfaces `trigger_refire_enabled` + new `rollout_step="step_5b_refire_live"` value. 29 new tests. No infinite-loop risk pinned by `test_refired_plan_does_not_re_park_in_subsequent_tick`. |

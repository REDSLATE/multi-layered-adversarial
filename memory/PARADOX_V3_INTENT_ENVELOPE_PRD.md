...
| 8 | ☐ Pending (~90d out) | — | After audit-retention window: **retire v2 fast-path emit to dormant/callable** (operator pin 2026-02-22) — NOT delete. The v2 emit path remains in the codebase so historical replay (running new v3 doctrine against old v2 behaviour years later) stays possible. Bridges switch to v3 as default emitter; v2 becomes a callable code path used only for back-testing + audit replay. |

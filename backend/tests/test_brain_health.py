...
def test_verdict_ignores_opinion_silence_for_executor_only_brain():
    """2026-02-17 doctrine pin: executor / crypto seats DO NOT post
    opinions — they route orders generated by the strategist. A brain
    whose only held seat is `executor` (e.g. Alpha) MUST NOT be
    flagged for opinion-silence. Mirroring the frontend tile's
    'n/a (executor)' display so the verdict logic and the visual
    label agree."""
    checkin = {"verdict": "prod", "age_sec": 30.0}
    opinion = {"silent": True, "age_sec": 99999.0}
    seat_walk = _stub_seat_walk([("executor", "equity")])
    v = mod._compute_overall(checkin, opinion, seat_walk)
    assert v["verdict"] == "green", (
        "An executor-only brain must not be flagged for opinion-"
        "silence. Executors route orders; opinions come from "
        "strategist / governor / auditor seats. "
        f"got reasons={v['reasons']!r}"
    )
    assert not any("opinion" in r for r in v["reasons"]), (
        f"No opinion-related reason should fire for executor-only; "
        f"got {v['reasons']!r}"
    )


def test_verdict_still_flags_opinion_silence_when_holding_strategist():
    """Counter-test for the executor exemption: a brain holding
    `strategist` (which IS an opinion-producing seat) MUST still be
    flagged when silent. The exemption is per-role, not blanket."""
    checkin = {"verdict": "prod", "age_sec": 30.0}
    opinion = {"silent": True, "age_sec": 5000.0}
    seat_walk = _stub_seat_walk([("strategist", "equity")])
    v = mod._compute_overall(checkin, opinion, seat_walk)
    assert v["verdict"] == "degraded", v
    assert any("opinion_silent" in r for r in v["reasons"]), (
        "Strategist seat IS opinion-producing — silence must flag."
    )

...
def test_ranking_selects_highest_setup_score_not_alphabetical():
    ...
    async def go():
        return await r._rank_universe(http=None)
    ranked = _run(go())
...
def test_cooldown_releases_after_window():
    ...
    pick = None
    for lane, symbol, _score in _run(r._rank_universe(http=None)):
        last = r._last_emit_tick.get((lane, symbol), -10**9)
        if (r._tick_count - last) < runner.INTENT_COOLDOWN_TICKS:
            continue
        pick = (lane, symbol)
...
def test_score_failures_degrade_not_drop():
    ...
    ranked = _run(r._rank_universe(http=None))

"""Crypto-lane take-profit lifecycle.

Wraps the lane-neutral `shared.risk.take_profit_guard` with crypto
position bookkeeping. Belongs to REDEYE's crypto executor lane per
doctrine: executors enter, lifecycle guards exit, brains advise,
RoadGuard enforces.

Reads:    `shared_live_positions` (lane='crypto' rows only)
Closes:   via `shared.live_positions.close()` — broadcasts to outcomes.
          Actual Kraken close-order routing remains a separate piece
...
async def evaluate_position(
    *,
    ...
    """Look up a crypto live position, run the deterministic guard,
    return the verdict — does NOT execute any close."""
...
async def enforce_position(
    *,
    ...
    """Evaluate AND act. HOLD = no-op. REDUCE = record_management with a
    negative-delta sized by `close_fraction`. CLOSE = terminal close
    with auto-derived pnl and outcome broadcast."""

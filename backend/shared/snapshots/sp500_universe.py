"""Static S&P 500 universe used by the daily-snapshot subsystem.

Pinned as a static list so daily-snapshot capture is deterministic
and reproducible across pod restarts. The S&P 500 changes slowly
(~10 constituents per year) — when an index reshuffle happens the
operator updates this list manually.

Doctrine:
  The list defines WHICH symbols the daily snapshot tries to
  freeze. A symbol present here but missing from
  `shared_ohlcv_bars` produces a snapshot row with `price: null`
  and `price_reason: "no_bars_for_symbol"` — the gap is auditable,
  never silently dropped.

Source: S&P 500 constituents, snapshot taken early 2026.
"""
from __future__ import annotations

# 500 ticker S&P 500 constituents (alphabetical by ticker for diffability).
# Update via PR when the index reshuffles; the list is intentionally
# code-level (not DB-seeded from an external source) so it's pinned
...
SP500_TICKERS: tuple[str, ...] = (
    ...
    "FITB", "FMC", "FOX", "FOXA", "FRT", "FSLR", "FTNT", "FTV", "GD", "GE",
...
assert len(SP500_TICKERS) >= 500, f"SP500_TICKERS list shrunk: {len(SP500_TICKERS)}"

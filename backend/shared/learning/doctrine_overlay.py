...
# ═══════════════════════════════════════════════════════════════════
#  Gate-tuning overlay — feedback loop from counterfactual signals
# ═══════════════════════════════════════════════════════════════════
#
# Approved `counterfactual_tuning_signals` land here as a bounded
# delta on gate thresholds, keyed by `(blocked_reason, lane)`.
# Positive delta = RELAX (lower gate threshold — let more through).
# Negative delta = TIGHTEN (raise threshold — block more).
#
# Same TTL cache pattern as the notional overlay. Hard-clamped to
# ±0.20 so a runaway approval can never rewrite the base doctrine.

# lazy-imported at first use to avoid a circular import at module load
_TUNING_COLL_NAME = "counterfactual_tuning_signals"

_gate_cache: dict[str, dict[str, Any]] = {}
_gate_cache_loaded_at: float = 0.0


def reload_gate_cache_for_tests() -> None:
    """Force the next call to reload gate tuning from Mongo. Tests only."""
    global _gate_cache_loaded_at
    _gate_cache_loaded_at = 0.0
    _gate_cache.clear()


def _gate_key(blocked_reason: str, lane: str) -> str:
    """Canonical key for the (blocked_reason, lane) lookup."""
    br = (blocked_reason or "unknown").lower()
    ln = (lane or "unknown").lower()
    return f"{br}|{ln}"


async def _ensure_gate_cache_fresh(db) -> None:
    """Reload `_gate_cache` from `counterfactual_tuning_signals` if
    the TTL has expired. Best-effort — hiccups leave prev cache."""
    global _gate_cache_loaded_at
    now = time.monotonic()
    if _gate_cache_loaded_at and (now - _gate_cache_loaded_at) < CACHE_TTL_SEC:
        return
    try:
        cur = db[_TUNING_COLL_NAME].find(
            {"state": "approved"},
            {
                "_id": 0, "group": 1, "kind": 1,
                "proposal": 1, "modifier": 1,
            },
        )
    except Exception as exc:  # noqa: BLE001
        logger.warning("doctrine_overlay: gate cache reload query failed: %s", exc)
        return
    ...
    try:
        async for row in cur:
            ...
            br = group.get("blocked_reason")
            lane = group.get("lane")
            if not br:
                continue
            key = _gate_key(br, lane)
            explicit = row.get("modifier")
            if explicit is None:
                explicit = (
                    row.get("proposal", {}).get("modifier")
            ...
            if explicit is None:
                if kind == "relax_gate":
                    modifier = 0.10  # default relax
                elif kind == "preserve_gate":
                    modifier = -0.10  # default preserve/tighten
                else:
                    continue
            else:
                try:
                    ...
                    continue
            new_cache[key] = {"kind": kind, "modifier": _clamp(modifier)}
    except Exception as exc:  # noqa: BLE001
        logger.warning("doctrine_overlay: gate cache reload iter failed: %s", exc)
        return

    _gate_cache.clear()
    _gate_cache.update(new_cache)
    _gate_cache_loaded_at = now


async def get_gate_threshold_delta(
    blocked_reason: str, lane: str, *, db,
) -> float:
    """Return the gate-threshold delta for `(blocked_reason, lane)`.

    Range: **-0.20 .. +0.20** — a raw signed modifier callers add
    to a base gate threshold.
        +Δ  = relax the gate (lower threshold; let more through)
        -Δ  = tighten the gate (raise threshold; block more)

    Default (no approved tuning signal matches): 0.0 (no change).

    Approved `counterfactual_tuning_signals` populate the cache.
    """
    if not blocked_reason:
        return 0.0
    await _ensure_gate_cache_fresh(db)
    hit = _gate_cache.get(_gate_key(blocked_reason, lane))
    if hit is None:
        return 0.0
    return _clamp(hit.get("modifier", 0.0))
